...
test('syncCard stamps pr_state and logs on change', () => {
  ...
  const res = syncCard(board, state, 'mb-1', { exec });
...
test('syncAll skips archived cards and cards without PR refs', () => {
  const state = makeState();
  ...
  const results = syncAll(board, state, { exec });
